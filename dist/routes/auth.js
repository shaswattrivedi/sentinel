...
router.post("/login", async (req, res, next) => {
    ...
    if (!parsed.success)
        return next(new HttpError(400, "AUTH_401", "Invalid credentials"));
    try {
...
router.post("/refresh", async (req, res, next) => {
    ...
    if (!parsed.success)
        return next(new HttpError(400, "AUTH_401", "Invalid refresh token"));
    try {
...
router.post("/signup", authenticate, requireRoles([UserRole.SUPER_ADMIN]), async (req, res, next) => {
    ...
    if (!parsed.success)
        return next(new HttpError(422, "ML_422", "Invalid signup payload"));
    try {
        const existing = await userService.findUserByEmail(parsed.data.email);
        if (existing)
            return next(new HttpError(409, "DATA_409", "User already exists"));
...
router.post("/signup/public", async (req, res, next) => {
    ...
    try {
        ...
        return res.status(201).json(success(req, {
            access_token: result.accessToken,
            refresh_token: result.refreshToken,
            token_type: "Bearer",
            user: result.user
